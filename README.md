# Hello, I'm Haaris! 👋

I'm a Master's student in *Applied Data Science & AI* at SRH University, Heidelberg, currently looking for *Werkstudent* opportunities (up to 20hrs/week) in Germany.

## 🎯 About Me

- 🎓 Pursuing M.Sc. in Applied Data Science and AI (2025-2027)
- 💼 2+ years of hands-on experience in data analytics, machine learning, and BI dashboards
- 🔍 Worked across multiple domains: Finance, Telecom, Manufacturing, AI/Data Operations 
- 📊 Strong focus on turning data into actionable insights through visualization and modeling
- 🌱 Currently improving my German and deepening my cloud infrastructure knowledge

## 🛠️ Technical Skills

*Programming & Data:* Python (Pandas, NumPy, Scikit-learn), SQL, Excel (Advanced functions, Power Query, Macros)

*Machine Learning:* Classification models, Random Forest, Feature Engineering, Model Evaluation

*Visualization & BI:* Power BI, Tableau, SAS, Matplotlib

*Cloud & Tools:* AWS, Azure basics, MLOps fundamentals

## 💻 What I've Worked On

- Built *churn prediction models* (89% accuracy) using Random Forest on 50K+ customer records
- Developed *12+ Power BI dashboards* with automated refresh, reducing reporting time by 30%
- Automated *data extraction pipelines* using Python and Selenium (35K+ data points daily)
- Engineered *Oracle SQL queries* for banking compliance reports with 100% accuracy
- Created *interactive dashboards* analyzing energy-weather correlations across 1M+ rows

## 🎓 Education

- *M.Sc. Applied Data Science & AI* - SRH University, Heidelberg (2025 - 2027)
- *B.Tech Electronics & Telecommunication* - Symbiosis Institute of Technology, Pune
  - *Relevant Coursework:* Data Structures & Algorithms, Object-Oriented Programming & Python, Data Science, Big Data Analytics, Engineering Mathematics I/II/III, Stochastics & Probability

## 📜 Certifications

- Google Data Analytics Specialization - Google
- Python for Data Science, AI & Development - IBM
- SQL for Data Science - University of California
- AWS Cloud Foundations - AWS Academy
- Data Visualization with Tableau Specialization - University of California

## 💼 Career Highlights

*Internships:* Wuerth India, Forage AI, In-Solutions Global (Banking), Tata Tele Business Services, Ellicium Solutions

*Domains:* Manufacturing, AI/Data Operations, Financial Services, Telecommunications, Data Analytics Consulting

*Impact Areas:* BI Dashboard Development, Data Pipeline Automation, SQL Optimization, Web Scraping, Data Quality Assurance

## 📫 Let's Connect

- 📧 Email: haariskhalil@gmail.com
- 💼 LinkedIn: [linkedin.com/in/haaris-khalil](https://www.linkedin.com/in/haaris-khalil/)
- 📍 Based in Mannheim, Germany

---

Open to Werkstudent positions in Data Analytics, Machine Learning, or BI Development!
